...
//        System.out.println(scnr.nextLine());
    ...
    int[] compressedRun = new int[rowsOftext];
    int[] compressedLength = new int[rowsOftext];
    int compCount = 1;
    int compRaw = 0;
    for (int r = 0; r < rowsOftext-1; r++) {
      if (discount[r] == discount[r + 1]) {
        compCount++;
      } else {
        compressedRun[compRaw] = discount[r];
        compressedLength[compRaw] = compCount;
        compCount=1;
        compRaw++;
      }
    }
    compressedRun[compRaw] = discount[rowsOftext-1];
    compressedLength[compRaw] = compCount;
