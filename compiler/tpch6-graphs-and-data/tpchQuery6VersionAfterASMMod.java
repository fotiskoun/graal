...
public class tpchQuery6VersionAfterASMMod {
...
//    System.out.println(scnr.nextLine());
    ...
    for(;;){
      callFunction(cacheSizeIntegers, dumpArray, i, di, qu, shipdate, endDate, startDate, extendedprice);
    }

  }

  public static void callFunction(int cacheSizeIntegers, int[] dumpArray, int i,  CompressedProxy di, CompressedProxy qu, int[] shipdate, int endDate, int startDate, int[] extendedprice){
    ...
    for (int iteration = 0; iteration < 15; iteration++) {
      ...
      for (i = 0; i < shipdate.length; ) {
        nextRes = getNextAlignment(cols, iters);
        if (nextRes[0][1] == -1) {
          length = di.startPosition[di.size] - curRes[1];
        } else {
          length = nextRes[0][1] - curRes[1];
        }
        if (curRes[0] <= 7) {
          if (curRes[0] >= 5) {
            if (curRes[2] < 24) {
              for (inner_i = 0; inner_i < length; inner_i++) {
                if (shipdate[inner_i + i] <= endDate) {
                  if (shipdate[inner_i + i] > startDate) {
                    sum += extendedprice[inner_i + i] * curRes[0];
                  }
                }
              }
              i += length;
